...
const EmblaCarousel = ({fansLikeDetail}: CarouselProps) => {
  ...
  const slides = fansLikeDetail && fansLikeDetail?.filter(
    ...
    ref={emblaRef} >
      <div>
        {slides.slice(0, 5).map((item) => (
          <figure
          key={item._id} className='carousel_box'
          onClick={() => router.push(`/podcasts/${item.podcast[0]?.podcastId}`)}>
            <Image src={item.imageUrl} alt='card'
            fill
            className='absolute size-full rounded-xl border-none'/>
            <div className='glassmorphism-black relative z-10 flex flex-col rounded-b-xl p-4'>
              <h2 className='text-14 font-semiblod text-white-1'>
